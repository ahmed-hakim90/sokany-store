  ...
  return (
    <section className={`${aboutLandingLightSectionClass} space-y-8`} aria-labelledby="about-service-title">
      ...
      <div className="space-y-2">
        ...
        <p className={cn("max-w-2xl", aboutLandingLeadClass)}>{subtitle}</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
        ...
        >
          ...
          <div className="absolute bottom-5 start-5 end-5 max-w-sm rounded-xl bg-brand-950/92 p-4 text-white shadow-lg backdrop-blur-sm sm:bottom-6 sm:start-6">
            ...
            <Link
              href={mapCta.href}
              className={aboutLandingOverlayCtaClass}
            >
              {mapCta.label}
            </Link>
          </div>
        ...
        <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          {regions.map(({ title: regionTitle, stat, description }) => (
            <li key={regionTitle}>
              <article className={cn(aboutLandingPanelClass, "flex h-full flex-col gap-1 p-4 sm:p-5")}>
