/**
 * Placeholder aligned with default catalog `ProductCard` image wells
 * (h-[138px] sm:h-[146px] lg:h-[154px]); compact rails may differ by ~2px — acceptable to avoid CLS.
 */
export function ProductSkeleton() {
  return (
    <div className="surface-product-card flex h-full min-w-0 flex-col gap-2 overflow-hidden rounded-2xl border-0 p-0 shadow-none ring-0">
      <div className="h-[138px] w-full animate-shimmer rounded-xl bg-gradient-to-r from-surface-muted via-background to-surface-muted bg-[length:200%_100%] sm:h-[146px] lg:h-[154px]" />
      <div className="flex flex-1 flex-col gap-1.5 px-2 pb-2 pt-1 sm:gap-2 sm:px-2.5 sm:pb-2.5 sm:pt-1.5">
        ...
        <div className="mt-auto grid grid-cols-[minmax(0,1fr)_auto] items-center gap-1.5 sm:gap-2">
          ...
          <div className="h-9 min-h-[2.75rem] w-14 animate-shimmer rounded-full bg-surface-muted/90 ring-1 ring-foreground/[0.04] sm:w-16" />
        </div>
      </div>
    </div>
  );
