...
export function CheckoutOtpModal({
  ...
  return open ? (
    <div
      className="fixed inset-0 z-[220] flex animate-fade-in items-end justify-center bg-black/55 p-4 motion-reduce:animate-none sm:items-center"
      role="presentation"
    >
      <div
        role="dialog"
        ...
        className="w-full max-w-md animate-slide-up overflow-hidden rounded-2xl border border-border bg-white shadow-2xl motion-reduce:animate-none"
      >
        <FocusTrap
          focusTrapOptions={{
          ...
          <div className="flex flex-col gap-4 p-6">
            ...
            <div className="flex flex-col gap-2 sm:flex-row-reverse sm:justify-between">
              <Button
                ...
                className="flex-1"
                loading={isVerifying}
                disabled={isVerifying || isSending || code.replace(/\s/g, "").length < 4}
                onClick={() => void submit()}
              >
                تأكيد الرمز
              </Button>
